...
static u08 CmdlineBuffer[CMDLINE_BUFFERSIZE];

u08 CmdlineBufferLength;
u08 CmdlineBufferEditPos;
u08 CmdlineInputVT100State;
static u08 CmdlineHistory[CMDLINE_HISTORYSIZE][CMDLINE_BUFFERSIZE];
...
void cmdlineInputFunc(unsigned char c)
{
	...
	if( (c >= 0x20) && (c < 0x7F) )
	{
		...
		if(CmdlineBufferEditPos == CmdlineBufferLength)
		{
			...
			SP5K_CmdlineBuffer[CmdlineBufferEditPos] = c;
			CmdlineBufferEditPos++;
			//->CmdlineBuffer[CmdlineBufferEditPos++] = c;
		...
		else
		{
			...
			for(i=CmdlineBufferLength; i>CmdlineBufferEditPos; i--) {
				...
				SP5K_CmdlineBuffer[i] = SP5K_CmdlineBuffer[i-1];
			}
			// insert character
			CmdlineBuffer[CmdlineBufferEditPos] = c;
			SP5K_CmdlineBuffer[CmdlineBufferEditPos] = c;
	...
	else if(c == ASCII_CR)
	{
		...
		SP5K_CmdlineBuffer[CmdlineBufferLength] = 0;
		CmdlineBufferLength++;
		//->CmdlineBuffer[CmdlineBufferLength++] = 0;
		CmdlineBufferEditPos++;
...
u08* SP5K_GetCmdlinePtr(void)
{

	return (&CmdlineBuffer);
}

void SP5K_GetLineArg( u08 *dstStr , u08 size)
{
u08 i;

	memcpy( dstStr, &CmdlineBuffer, size);
